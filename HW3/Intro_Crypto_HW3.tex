...
\begin{solution}
  ...
  Finalmente, cuando Alice envía $w$, el adversario computa $k_B := w \oplus t$. Y así consigue armar la clave de Bob y, por lo tanto, de Alice.
...
\begin{solution}
  ...
  Definimos CDH de la siguiente manera. El problema CDH es difícil relativo a $\mathcal{G}$ si para todo adversario PPT $\mathscr{A}$, el experimento descrito arriba tiene probabilidades negligibles de tener éxito. Es decir,
  $$\Pr[CDH_{\mathscr{A}, \mathcal{G}}(n) = 1] \leq negl(n)$$
...
Asumimos que tenemos un adversario $\mathscr{A}_{DLOG}$ que resuelve el problema del logaritmo discreto con probabilidad no negligible (es decir, dados $(\mathbb{G}, q, g, h)$, puede devolver $x$ tal que $g^x = h$). 
  ...
  $$\left| \Pr[\mathcal{A}(\mathbb{G}, q, g, g^{x_1}, g^{x_2}, g^z) = 1] - \Pr[\mathcal{A}(\mathbb{G}, q, g, g^{x_1}, g^{x_2}, g^{x_1\cdot x_2}) = 1] \right| \leq \mathsf{negl}(n)$$
   ...
   \begin{enumerate}
    ...
    \item Computa $h := CDH(\mathbb{G}, q, g, g^{x_1}, g^{x_2})$. Con probabilidad no negligible, $h = g^{x_1 \cdot x_2}$.
    \item Devuelve $1$ si $g^k = h$, $0$ si no.
   \end{enumerate}

   Veamos que este adversario distingue con probabilidad no negligible. Si la entrada es del tipo $(g^{x_1}, g^{x_2}, g^{x_1 \cdot x_2})$, entonces cuando CDH tiene éxito (con probabilidad no negligible) se cumple $h = g^{x_1 \cdot x_2} = g^k$, y $\mathscr{A}_{DDH}$ devuelve $1$. En cambio, si la entrada es $(g^{x_1}, g^{x_2}, g^z)$ con $z$ uniforme, la probabilidad de que $g^z = h$ es $1/q$ (negligible). Luego, la diferencia de probabilidades es no negligible, contradiciendo la dificultad de DDH.
...
\begin{parts}
    ...
    \begin{solution}
      ...
      Luego, $y = y' + 1$. Veamos qué implica esto para $c_2$ y $c'_2$.
...
\begin{solution}
  Elegimos $r \in \mathbb{Z}_q$ uniforme (con $r \neq 0$) y definimos
  $$c'_1 := c_1 \cdot g^r, \qquad c'_2 := c_2 \cdot \alpha \cdot h^r.$$

  Ambos son computables a partir de la información pública ($g$, $h$, $\alpha$) y del ciphertext original $\langle c_1, c_2\rangle$. Notemos que $c'_1 \neq c_1$ pues $r \neq 0$, y $c'_2 \neq c_2$ siempre que $\alpha \cdot h^r \neq 1$, lo cual ocurre con probabilidad abrumadora sobre la elección de $r$.

  Verifiquemos que $\langle c'_1, c'_2\rangle$ desencripta a $\alpha \cdot m$. Recordemos que $h = g^x$, $c_1 = g^y$ y $c_2 = h^y \cdot m$.
  $$Dec_{SK}(\langle c'_1, c'_2 \rangle) = \frac{c'_2}{(c'_1)^x} = \frac{c_2 \cdot \alpha \cdot h^r}{(c_1 \cdot g^r)^x} = \frac{h^y \cdot m \cdot \alpha \cdot h^r}{g^{xy} \cdot g^{xr}} = \frac{h^{y+r} \cdot \alpha \cdot m}{h^{y+r}} = \alpha \cdot m.$$

  Es decir, $\langle c'_1, c'_2\rangle$ es una encripción válida de $\alpha \cdot m$ con $c'_1 \neq c_1$ y $c'_2 \neq c_2$, para cualquier $\alpha$ conocido.
...
\begin{solution}
  Veamos primero que $e$ tiene que ser impar. Para empezar, vemos que $\phi(N) = (p-1)(q-1)$, por lo tanto es par, ya que $p$ y $q$ son números primos grandes (por ende ninguno es $2$, el único primo par). Luego, como las condiciones de $e$ son que sea mayor a $1$ y que $gcd(e, \phi(N)) = 1$, $e$ no puede ser par. Si lo fuese, compartiría un divisor con $\phi(N)$ y el $gcd$ no sería $1$. 

  Veamos ahora lo mismo para $d$. Como $d$ se define en base a $e$, vamos a usar que $e$ es impar. Dado que $d := e^{-1} \bmod \phi(N)$, podemos expresarlo como que $e \cdot d \equiv 1 \pmod{\phi(N)}$. Es decir, existe un $k$ tal que $e \cdot d = k \cdot \phi(N) + 1$. Luego, vemos que el lado derecho es impar. Como $e$ es impar, el lado izquierdo solamente puede ser impar si $d$ es impar. Luego, $d$ es impar por definición.
\end{solution}
...
\begin{solution}
  Vemos que no es CCA-seguro. Para verlo, definimos un adversario que tiene éxito con probabilidad mayor o igual a $\frac{1}{2} + negl(n)$ en el experimento de $PubK$:
  \begin{enumerate}
    \item Se corre $Gen(1^n)$ para obtener las claves $(pk, sk)$. En este caso son $(\langle N, e\rangle, d)$
    \item El adversario $\mathbb{A}$ recibe $pk$ y acceso al oráculo $Dec_{sk}$. Devuelve un par de mensajes $m_0$ y $m_1$ de igual longitud en el espacio de mensajes relacionado con $pk$.
    \item Se elige de manera uniforme un bit $b \in \{0, 1\}$ y se envía $c := Enc_{pk}(m_b)$ a $\mathbb{A}$
    \item El adversario puede interactuar con el oráculo, pero no puede pedirle $Dec_{sk}(c)$. Luego, $\mathbb{A}$ devuelve $b'$
    \item El resultado del experimento es $1$ si $b' = b$. 
  \end{enumerate}

  El adversario va a hacer lo siguiente. Dado que elige $m_0$ y $m_1$, va a computar $c_0$ y $c_1$ tal que $c_i := (H(m_i) || m_i)^e \bmod N$. 
  Luego, cuando recibe $c$, lo compara con ambos y devuelve $b'=0$ si $c_0 = c$ y $b'=1$ si $c_1 = c$. 

  Luego, esto lo hace con probabilidad $1$, ya que $H$ es determinística.

  Luego, el esquema no es CCA-seguro. 
